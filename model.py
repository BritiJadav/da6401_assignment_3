...
Autograder evaluation pattern (every test sample):
    model = Transformer().to(device)
    model.eval()
    english_sentence = model.infer(german_sentence)

Design decisions to satisfy all constraints:
  • ALL default argument values are set so Transformer() works with zero args
  • Vocabulary building + spaCy tokenizer loading happens inside __init__
  • Trained weights are downloaded via gdown inside __init__
  • infer() is a pure end-to-end method: raw German str → English str
"""
...
# ══════════════════════════════════════════════════════════════════════
#  VOCABULARY  (self-contained copy; mirrors dataset.py so model.py
#              is fully standalone for the autograder)
# ══════════════════════════════════════════════════════════════════════
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            mask                : BoolTensor broadcastable to
...
# ══════════════════════════════════════════════════════════════════════
#  GDRIVE WEIGHT CONFIGURATION
#  ─────────────────────────────────────────────────────────────────────
#  After training:
#    1. Upload best_checkpoint.pt to Google Drive
#    2. Share → "Anyone with the link → Viewer"
#    3. Copy the file-id from the share URL:
#         https://drive.google.com/file/d/<FILE_ID>/view
#    4. Paste it into GDRIVE_FILE_ID below
# ══════════════════════════════════════════════════════════════════════
...
class Transformer(nn.Module):
    ...
    ALL arguments have sensible defaults so the autograder can call:
        model = Transformer().to(device)
        model.eval()
        english = model.infer(german_sentence)

    What __init__ does automatically (no caller intervention needed):
        1. Load spaCy tokenisers  (de_core_news_sm, en_core_web_sm)
        2. Build src/tgt vocabularies from Multi30k training split
        3. Construct the nn architecture with the derived vocab sizes
        4. Download trained weights from Google Drive via gdown
        5. Load those weights into the model
    ...
    Args:
        ...
        max_infer_len  (int)  : Max tokens to generate          (default 100)
        weights_path   (str)  : Local path for checkpoint file
        min_freq       (int)  : Vocabulary minimum token freq   (default 2)
    """
    ...
    def __init__(
        ...
        # ── STEP 1: store inference-time hyper-params ─────────────────
        # (before super().__init__() so they're available immediately)
    ...
    def _load_spacy_model(model_name: str):
        """Load a spaCy model, downloading it first if not installed."""
        import spacy
        try:
    ...
    def _build_vocabs(self, min_freq: int = 2) -> Tuple[Vocabulary, Vocabulary]:
        """
        Build source (de) and target (en) vocabularies from the
        Multi30k training split downloaded via HuggingFace datasets.
        """
        from datasets import load_dataset
        train_data = load_dataset('bentrevett/multi30k', trust_remote_code=True)['train']
    ...
    def _download_and_load_weights(self) -> None:
        """
        Download the trained checkpoint from Google Drive (using gdown)
        if it is not already present locally, then load it into this model.

        The file-id must be set in GDRIVE_FILE_ID at the top of this file.
        """
        if GDRIVE_FILE_ID == "YOUR_GDRIVE_FILE_ID_HERE":
            print(
                "[Transformer] WARNING: GDRIVE_FILE_ID is not configured.\n"
                "  Set GDRIVE_FILE_ID at the top of model.py before submission.\n"
                "  Skipping weight download — model will use random initialisation."
            )
            return
        ...
        if not os.path.exists(self._weights_path):
            ...
            try:
                ...
                print("[Transformer] gdown not found — installing...")
                subprocess.run(
        ...
        # Support both raw state_dict and our save_checkpoint() format
        state_dict = (
    ...
    # ─────────────────────────────────────────────────────────────────
    #  CORE FORWARD METHODS  (autograder contract — do not rename)
    # ─────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────
    #  END-TO-END INFERENCE  (autograder contract — do not rename)
    # ─────────────────────────────────────────────────────────────────
    ...
    def infer(self, german_sentence: str) -> str:
        ...
        Full pipeline:
            1. Tokenise the German input with spaCy (de_core_news_sm)
            2. Convert tokens → integer indices using src_vocab
            3. Build the source padding mask
            4. Encode the source with the Transformer encoder
            5. Autoregressively generate English tokens (greedy decoding)
               — starting from <sos>, stopping at <eos> or max_infer_len
        ...
        Returns:
            english_sentence (str): Predicted English translation, e.g.
        ...
        # ── 2. Encode tokens → tensor ─────────────────────────────────
        src = torch.tensor(
            self.src_vocab.encode(tokens),   # adds <sos> and <eos>
            dtype=torch.long,
            device=device,
        ...
        with torch.no_grad():
            ...
            # ── 5. Greedy autoregressive decoding ─────────────────────
