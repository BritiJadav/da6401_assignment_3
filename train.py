...
def run_epoch(
    ...
    with ctx:
        ...
        for src, tgt in pbar:
            ...
            tgt_input  = tgt[:, :-1]   # decoder input  (drop last token)
            tgt_output = tgt[:, 1:]    # prediction target (drop <sos>)
...
def beam_search_decode(
    ...
    with torch.no_grad():
        ...
        memory   = memory.expand(beam_size, -1, -1)       # [B, src_len, d_model]
        src_mask = src_mask.expand(beam_size, -1, -1, -1) # [B, 1, 1, src_len]
...
def run_training_experiment() -> None:
    ...
    if os.path.exists(best_ckpt_path):
        ...
        start_epoch = load_checkpoint(best_ckpt_path, model, optimizer, scheduler)
        # Re-evaluate val loss of the loaded checkpoint so we don't
        # overwrite it with a worse model immediately
        best_val_loss = run_epoch(val_loader, model, loss_fn,
                                  None, None, epoch_num=start_epoch,
                                  is_train=False, device=device)
        print(f"Resumed val loss: {best_val_loss:.4f}")

    total_epochs = start_epoch + cfg.num_epochs   # train for num_epochs MORE epochs
